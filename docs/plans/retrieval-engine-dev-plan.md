...
- **Tracing coverage is partial.** As of Phase 4, OTLP + manual spans exist on **query-service** and **reranker-service** only. Batch CLIs (`eval`, `ingest`, `embed`, `index-fts`) do not call `setup_telemetry()` yet — eval runs produce no Jaeger traces. Redis/personalization (4.5) and itinerary (4.6) services need tracing from day one. See [docs/handoff/phase-4.5.md](handoff/phase-4.5.md) for the full gap table; close eval CLI tracing in 4.5, ingestion/embed in Phase 5, full end-to-end in Phase 6.
