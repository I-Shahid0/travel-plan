...
- **Tracing coverage is complete** as of Phase 6: all services and batch CLIs export OTLP; degradation is visible in both traces (`circuit_open`, `served_fallback` span attributes) and metrics (`circuit_breaker_state` gauge). Metrics go straight to Prometheus (app `/metrics` endpoints); the Collector stays traces-only. See [docs/handoff/phase-7.md](handoff/phase-7.md).
