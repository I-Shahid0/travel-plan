...
- **Tracing coverage is complete** as of Phase 6: all services and batch CLIs export OTLP; degradation is visible in both traces (`circuit_open`, `served_fallback` span attributes) and metrics (`circuit_breaker_state` gauge). Metrics go straight to Prometheus (app `/metrics` endpoints); the Collector stays traces-only. **Phase 7** added tail-based sampling in the Collector (errors, slow traces, degradation spans, 10% baseline). See [docs/handoff/phase-7.md](handoff/phase-7.md).
- **Phase 6 prove-it is code-complete but the measured NDCG cost of degradation (`uv run eval --degradation`) still needs a corpus** — the dev database was empty at the end of the Phase 6 session. See the environment-gap note in [docs/handoff/phase-7.md](handoff/phase-7.md).
